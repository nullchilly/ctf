...
def main():
  ...
  print(t.get_vm())
  print(t.get_vm()['ctf_key'])
  targets = t.get_targets(10003)
  print(targets)
  for target in targets:
    if target['team_name'] == "UET.binary":
        continue
    print(target['team_name'], end = ' ')
    IP = target['hostname']
    PORT = 20003
    api_url = f"http://{IP}:{PORT}/api"
    exhibit_id = target['flag_id'][:36]
    museum_id = target['flag_id'][36:]
    print(exhibit_id, museum_id)
    ...
    with open("museum.log", "a") as f:
        f.write(t.submit_flag([resp["exhibits"][0]["metadata"]])[0])
